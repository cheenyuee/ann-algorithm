...
namespace glass {
    ...
    /**
     * HNSWInitializer 用来记录 hnsw 0 层以上的图
     */
struct HNSWInitializer {
  int N, K; // 数据点总数和每层的最大出度
  int ep; // entry point
  std::vector<int> levels; // levels[i] 记录第 i 个点的层高
  std::vector<std::vector<int, align_alloc<int>>> lists; // lists[i] 记录第 i 个点每层的邻居
